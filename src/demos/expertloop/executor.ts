// Deterministic local executor (port of expertloop/executor/run.py): a
// rule-following stand-in for the agent that receives the instruction set.
// It walks the compiled steps against a scenario, applies decision rules,
// records every action and tool call, and stops when a rule says so. Test
// cases assert on that trace.

import { STOP_WORDS, type DecisionRule, type InstructionDocument } from './compile';
...
export function execute(document: InstructionDocument, scenario: Scenario): ExecutionTrace {
  const trace: ExecutionTrace = { actions: [], tool_calls: [], rules_fired: [], outcomes: [], skipped: [], halted_at: null };
