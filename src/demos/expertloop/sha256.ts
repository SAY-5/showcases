// Synchronous SHA-256 and HMAC-SHA256 over UTF-8 strings: source content
// hashes for citations and the signature on every webhook delivery.
...
export function sha256(text: string): string {
  return toHex(digest(encoder.encode(text)));
}
...
export function hmacSha256(secret: string, message: string): string {
  ...
  inner.set(msg, 64);
  outer.set(digest(inner), 64);
  return toHex(digest(outer));
}
