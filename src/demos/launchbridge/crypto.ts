// Synchronous SHA-256 and HMAC-SHA256 over UTF-8, so signing and verification
// run inside the simulation step instead of awaiting the browser's async
// digest API. Output matches hashlib.sha256 and hmac.new(..., sha256).
...
export function sha256(data: Uint8Array): Uint8Array {
  ...
  for (let off = 0; off < padded.length; off += 64) {
    ...
    for (let i = 0; i < 64; i++) {
      ...
      const sa = ((a >>> 2) | (a << 30)) ^ ((a >>> 13) | (a << 19)) ^ ((a >>> 22) | (a << 10));
      const se = ((e >>> 6) | (e << 26)) ^ ((e >>> 11) | (e << 21)) ^ ((e >>> 25) | (e << 7));
      const t1 = (hh + se + ch + K[i] + w[i]) | 0;
      const t2 = (sa + maj) | 0;
      hh = g;
      g = f;
      f = e;
...
export function hmacSha256(key: string, message: string): string {
  ...
  const innerData = new Uint8Array(64 + msg.length);
  innerData.set(inner);
  innerData.set(msg, 64);
